...
grinchum-land:~$ 
```
This URL (recommended on Discord) was very helpful: https://book.hacktricks.xyz/linux-hardening/privilege-escalation/docker-breakout/docker-breakout-privilege-escalation
Working through the options, the most promising one was "Mounting Disk #1" . Per the description, a well configured docker image should not allow "fdisk -l", i.e. listing the partitions on the disk.

Running this command gives us visibility to sector "vda"
``` shell
...
Now, we cd into the directory (really, the sector) and the world is open to us.
```shell
...
Also, get familiar with tmux to split panes and switch between them - it will be useful: https://linuxize.com/post/getting-started-with-tmux/. In particular:
...
cd elf-ssh
```
Having a ssh-git connection gives us more power because we can now run actions, e.g. commits, with accepted credentials as opposed to just reading the code. Let's see what is kicked off by CI/CD when we commit - look at .gitlab.ci.yml (a hidden file at the root of the repo) using `$ cat ~/elf-ssh/.gitlab-ci.yml` 
...
```
At this point, if we haven't yet created another terminal and split the panes, do so (run tmux, then Ctrl-B-""). Our plan of attack is to park a listener on our server, and create a reverse shell during deployment that will connect with our listener. 
